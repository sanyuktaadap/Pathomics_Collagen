...
for analysis_win in sizes:
    ...
    # for i in range(0, 3, 2):
        ...
        csv_1 = pd.read_csv(f'data/hari_BC/otsu/otsu_patient_feats/{cohort}_cohort/{p1}_H&E_Breast_XXXXXXXX.csv', header=None)
        csv_2 = pd.read_csv(f'data/hari_BC/otsu/otsu_patient_feats/{cohort}_cohort/{p2}_H&E_Breast_XXXXXXXX.csv', header=None)
    ...
    df_black.to_csv(f"data/hari_BC/csv/{prefix}otsu_black_cohort_differences.csv", index=False)
    df_white.to_csv(f"data/hari_BC/csv/{prefix}otsu_white_cohort_differences.csv", index=False)
    ...
    plt.tight_layout()
    plt.savefig(f"{prefix}otsu_BvW_comparison.png", dpi=300)